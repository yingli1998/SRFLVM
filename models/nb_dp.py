...
class RFF_RFLVM_NB(nn.Module):
    def __init__(self, num_batch, num_sample_pt, param_dict, Y, device=None, ifPCA=True):
        ...
        self.R = nn.Parameter(10. * torch.ones(self.M), requires_grad=False)
    ...
    def _b_func(self, m=None):
        if m is not None:
            return self.Y[:, m] + self.R[m]
        return self.Y + self.R
    
    def _log_c_func(self):
        return torch.lgamma(self.Y + self.R) - torch.lgamma(self.Y + 1) - torch.lgamma(self.R)
    ...
    def compute_loss(self, batch_y, kl_option):
        ...
        self.omega = torch.tensor(self.sample_omega(b, psi, self.omega))
        ...
        for m in range(self.M):
            ...
            self.A[m] = torch.tensor(joint_sample)
        
        # reconstruction_term 
        phi_a = Phi @ self.A.T
        LL  = self._log_c_func() + self._a_func() * phi_a - self._b_func() * torch.log(1 + torch.exp(phi_a))
